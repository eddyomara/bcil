...
class gen_sale(models.TransientModel):
	...
	def make_sale(self, values):
		...
		if sale_search:
			...
			sales_team_id  = self.find_team(values.get('sales_team'))
	...
	def make_order_line(self, values, sale_id):
		...
		so_order_lines = order_line_obj.create({
											...
											'route_id':self.find_route(values.get('route')).id,
	...
	def find_route(self, name):
		...
		route_search = route_obj.search([('name', '=', name)])
		if route_search:
			return route_search

		else:
			raise Warning(_(' "%s" Route is not available.') % name)

	@api.multi
	def find_team(self, name):
		team_obj = self.env['crm.team']
		team_search = team_obj.search([('name', '=', name)])
		if team_search:
			return team_search
		else:
			raise Warning(_(' "%s" Sales team is not available.') % name)
	...
	def import_sale(self):
		...
		if self.import_option == 'csv':
			keys = ['order', 'customer', 'date', 'pricelist','route', 'product', 'quantity', 'price', 'uom', 'user', 'sales_team' ,'status']
			try:
