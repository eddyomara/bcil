...
class gen_pos_order(models.TransientModel):
    ...
    @api.multi
    def find_pricelist(self, pricelist ):    
        if pricelist:
            pricelist_ids  = self.env['product.pricelist'].search([('name', '=', pricelist )])
        if pricelist_ids:
            pricelist_ids = pricelist_ids[0]                     
            return pricelist_ids
        else:
            raise Warning(_('Wrong Pricelist %s') % pricelist)
    ...
    def make_pos(self, values):
        ...
        pricelist_id  = self.find_pricelist(values.get('pricelist'))             
        if partner_id and salesperson_id and session_id:
            ...
            if pos_search:
                ...
                pos_id = pos_obj.create({
                                        ...
                                        'name' : values.get('order_reference'),
                                        ...
                                        'amount_return' : 0.0,
                                        'amount_tax' : 0.0,
                                        'pos_reference' : values.get('receipt_reference'),
    ...
    def import_pos_order(self):
        if  self.import_option == 'csv':
                keys = ['name','order_reference','receipt_reference','session','date_order','salesperson','partner_id','product_id','quantity','price_unit','pricelist','discount','tax']
                try:
            ...
            for row_no in range(sheet.nrows):
                ...
                if row_no <= 0:
                    ...
                    values =  {     
                                'name':line[0],
                                'order_reference':line[1],
                                'receipt_reference':line[2],
                                'session': line[3],
                                'date_order': line[4],
                                'salesperson': line[5],
                                'partner_id': line[6],
